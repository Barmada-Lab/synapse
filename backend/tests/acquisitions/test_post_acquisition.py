...
@pytest.mark.asyncio
async def test_post_acquisition_flow_sync_fails_with_rollback(
    random_acquisition_dir: Path,
):
    experiment_id = random_acquisition_dir.name
    ...
    with patch("app.acquisitions.flows._sync_cmd") as mock_sync:
        ...
        with pytest.raises(AggregateError):
            await post_acquisition_flow(experiment_id=experiment_id, rollback=True)

    assert (
        acquisition_path.exists()
    ), "Acquisition path should not be deleted if sync fails"
    assert not analysis_path.exists(), "Analysis path should be deleted if sync fails"
    assert archive_path.exists(), "Archive path should created even if sync fails"


@pytest.mark.asyncio
async def test_post_acquisition_flow_sync_fails_without_rollback(
    random_acquisition_dir: Path,
):
    experiment_id = random_acquisition_dir.name
    ...
    with patch("app.acquisitions.flows._sync_cmd") as mock_sync:
        mock_sync.side_effect = create_file_and_raise_error(analysis_path)
        with pytest.raises(AggregateError):
            await post_acquisition_flow(experiment_id=experiment_id, rollback=False)
...
@pytest.mark.asyncio
async def test_post_acquisition_flow_archive_fails_with_rollback(
    random_acquisition_dir: Path,
):
    experiment_id = random_acquisition_dir.name
    ...
    ), "Archive path should be deleted if archiving fails"


@pytest.mark.asyncio
async def test_post_acquisition_flow_archive_fails_without_rollback(
    random_acquisition_dir: Path,
):
    experiment_id = random_acquisition_dir.name
...
@pytest.mark.asyncio
async def test_post_acquisition_flow_sync_and_archive_fails_with_rollback(
    random_acquisition_dir: Path,
):
    ...
    with (
        ...
        mock_archive.side_effect = create_file_and_raise_error(archive_path)
        with pytest.raises(AggregateError):
            await post_acquisition_flow(
                experiment_id=random_acquisition_dir.name, rollback=True
            )
...
@pytest.mark.asyncio
async def test_post_acquisition_flow_sync_and_archive_fails_without_rollback(
    random_acquisition_dir: Path,
):
    acquisition_path = settings.ACQUISITION_DIR / random_acquisition_dir.name
    analysis_path = settings.ANALYSIS_DIR / random_acquisition_dir.name
    ...
    with (
        ...
        with pytest.raises(AggregateError):
            await post_acquisition_flow(
                experiment_id=random_acquisition_dir.name, rollback=False
            )
